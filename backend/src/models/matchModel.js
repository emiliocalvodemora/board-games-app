...
export const getMatchByIdService = async (id) => {
    const result = await pool.query(
        `SELECT m.*, g.name as game_name, g.max_players FROM matches m 
        JOIN games g ON m.game_id = g.id WHERE m.id = $1`, [id]);
    return result.rows[0];
};

export const createMatchService = async (gameId, eventId, startTime, endTime) => {
    const result = await pool.query(
        `INSERT INTO matches (game_id, event_id, start_time, end_time) 
...
export const updateMatchService = async (id, gameId, eventId, startTime, endTime) => {
    const result = await pool.query(
        `UPDATE matches SET game_id = $1, event_id = $2, start_time = $3, end_time = $4
